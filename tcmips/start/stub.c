...
ssize_t read(int fd, void *buf, size_t nbyte) {
  if (fd == 0 && nbyte >= 1) {
    if (nbyte >= 2) {
      char *p = buf;
      uint32_t sz = tcm_text_console_read_string(buf, nbyte);
      p[sz] = '\n'; // todo: not icanon mode
      return ((ssize_t)sz + 1);
    }
    if (nbyte == 1) {
      ((char *)buf)[0] = tcm_text_console_read_char();
      return 1;
    }
  }
  return 0;
...
clock_t times(struct tms *buf) {
  uint32_t tk = tcm_syscall_get_tickcount();
  buf->tms_utime = tk;
  buf->tms_stime = 0;
  buf->tms_cutime = 0;
  buf->tms_cstime = 0;
...
int usleep(useconds_t __useconds) { // NOLINT
  ...
  uint32_t current;
  do {
    current = tcm_syscall_get_timestamp_micro();
  } while (current - start < __useconds);
