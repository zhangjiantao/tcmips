...
/* Perform a critical factorization of NEEDLE, of length NEEDLE_LEN.
   ...
   *PERIOD to the global period of the right half.
   ...
   periodicity.  */
static size_t
critical_factorization(const unsigned char *needle, size_t needle_len, size_t *period)
{
                     ...
                     const unsigned char *needle, size_t needle_len)
{
    ...
    if (CMP_FUNC(needle, needle + period, suffix) == 0) {
        ...
        while (AVAILABLE(haystack, haystack_len, j, needle_len)) {
            ...
            i = MAX(suffix, memory);
            while (i < needle_len && (CANON_ELEMENT(needle[i]) == CANON_ELEMENT(haystack[i + j])))
                ++i;
            if (needle_len <= i) {
...
two_way_long_needle(const unsigned char *haystack, size_t haystack_len, const unsigned char *needle,
                    size_t needle_len)
{
    size_t i;                           /* Index into current byte of NEEDLE.  */
    size_t j;                           /* Index into current window of HAYSTACK.  */
