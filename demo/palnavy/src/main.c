...
--*/
{
  ...
  VIDEO_SetWindowTitle(UTIL_va(
      UTIL_GlobalBuffer(0), PAL_GLOBAL_BUFFER_SIZE, "Pal %s%s%s%s",
...
--*/
{
  ...
  //
  // If user requests a file-based log, then add it after the system-specific
  // one.
  //
  ...
  UTIL_LogOutput(LOGLEVEL_INFO, "%s: %s\n", __func__, "PAL_Init ok");
  //
