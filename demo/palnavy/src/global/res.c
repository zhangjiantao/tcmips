...
--*/
{
  ...
  if (gpResources->bLoadFlags & kLoadScene) {
    ...
    UTIL_LogOutput(LOGLEVEL_INFO, "%s %d\n", __FUNCTION__, __LINE__);
    gpResources->lpMap =
        PAL_LoadMap(gpGlobals->g.rgScene[i].wMapNum, fpMAP, fpGOP);
  ...
  if (gpResources->bLoadFlags & kLoadPlayerSprite) {
    ...
    for (i = 1; i <= gpGlobals->nFollower; i++) {
      ...
      //
      wSpriteNum =
          gpGlobals->rgParty[(short)gpGlobals->wMaxPartyMemberIndex + i]
              .wPlayerRole;

      l = PAL_MKFGetDecompressedSize(wSpriteNum, gpGlobals->f.fpMGO);

      gpResources
          ->rglpPlayerSprite[(short)gpGlobals->wMaxPartyMemberIndex + i] =
          (LPSPRITE)UTIL_malloc(l);

      PAL_MKFDecompressChunk(
          gpResources
              ->rglpPlayerSprite[(short)gpGlobals->wMaxPartyMemberIndex + i],
          l, wSpriteNum, gpGlobals->f.fpMGO);
    }
  }
