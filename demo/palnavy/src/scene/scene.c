...
static VOID PAL_AddSpriteToDraw(LPCBITMAPRLE lpSpriteFrame, int x, int y,
                                int iLayer)
/*++
   Purpose:
...
--*/
{
  ...
  //
  for (i = 0;
       i <= (short)gpGlobals->wMaxPartyMemberIndex + gpGlobals->nFollower;
       i++) {
...
--*/
{
  ...
  int blockX = PAL_X(gpGlobals->partyoffset) / 32,
      blockY = PAL_Y(gpGlobals->partyoffset) / 16;
  ...
  if (fCheckEventObjects) {
    ...
    int i;
    for (i = gpGlobals->g.rgScene[gpGlobals->wNumScene - 1].wEventObjectIndex;
         i < gpGlobals->g.rgScene[gpGlobals->wNumScene].wEventObjectIndex;
         i++) {
...
--*/
{
  ...
  if (fWalking) {
    ...
    if (gpGlobals->g.PlayerRoles
            .rgwWalkFrames[gpGlobals->rgParty[0].wPlayerRole] == 4) {
    ...
    //
    i = gpGlobals->g.PlayerRoles
            .rgwWalkFrames[gpGlobals->rgParty[0].wPlayerRole];
    if (i == 0) {
      i = 3;
    }
    ...
    for (i = 1; i <= (short)gpGlobals->wMaxPartyMemberIndex; i++) {
      int f = gpGlobals->g.PlayerRoles
                  .rgwWalkFrames[gpGlobals->rgParty[i].wPlayerRole];
      if (f == 0) {
        f = 3;
      }
...
--*/
{
  ...
  if (g_InputState.dir != kDirUnknown) {
    xOffset =
        ((g_InputState.dir == kDirWest || g_InputState.dir == kDirSouth) ? -16
                                                                         : 16);
    yOffset =
        ((g_InputState.dir == kDirWest || g_InputState.dir == kDirNorth) ? -8
                                                                         : 8);
...
--*/
{
  ...
  //
  p->x += ((p->wDirection == kDirWest || p->wDirection == kDirSouth) ? -2 : 2) *
          iSpeed;
  p->y += ((p->wDirection == kDirWest || p->wDirection == kDirNorth) ? -1 : 1) *
          iSpeed;
