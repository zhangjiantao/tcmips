...
static unsigned int yj1_get_bits(const void *src, unsigned int *bitptr,
                                 unsigned int count) {
  ...
  *bitptr += count;
  if (count > 16 - bptr) {
    ...
    return (((unsigned short)((temp[0] | (temp[1] << 8)) << bptr)) >>
            (16 - count));
}
...
static unsigned short yj1_get_loop(const void *src, unsigned int *bitptr,
  ...
  if (yj1_get_bits(src, bitptr, 1))
    ...
    if (temp)
      return yj1_get_bits(src, bitptr,
                          header->CodeCountCodeLengthTable[temp - 1]);
    else
...
static unsigned short yj1_get_count(const void *src, unsigned int *bitptr,
  ...
  if ((temp = yj1_get_bits(src, bitptr, 2)) != 0) {
    if (yj1_get_bits(src, bitptr, 1))
      return yj1_get_bits(src, bitptr,
                          header->LZSSRepeatCodeLengthTable[temp - 1]);
    else
...
INT YJ1_Decompress(LPCVOID Source, LPVOID Destination, INT DestSize) {
  ...
  do {
    ...
    if ((node = root = (YJ1_TreeNode *)malloc(sizeof(YJ1_TreeNode) *
                                              (tree_len + 1))) == NULL)
      return -1;
    root[0].leaf = 0;
    root[0].value = 0;
  ...
  for (i = 0; i < SDL_SwapLE16(hdr->BlockCount); i++) {
    ...
    for (;;) {
      ...
      while (loop--) {
        ...
        pos =
            yj1_get_bits(src, &bitptr, header->LZSSOffsetCodeLengthTable[pos]);
        while (count--) {
          *dest = *(dest - pos);
          dest++;
        }
...
static unsigned char yj2_data1[0x100] = {
    };
static unsigned char yj2_data2[0x10] = {0x08, 0x05, 0x06, 0x04, 0x07, 0x05,
                                        0x06, 0x03, 0x07, 0x05, 0x06, 0x04,
                                        0x07, 0x04, 0x05, 0x03};
...
static int yj2_build_tree(YJ2_Tree *tree) {
  ...
  YJ2_TreeNode *node;
  if ((tree->list = list =
           (YJ2_TreeNode **)malloc(sizeof(YJ2_TreeNode *) * 321)) == NULL)
    return 0;
  if ((tree->node = node =
           (YJ2_TreeNode *)malloc(sizeof(YJ2_TreeNode) * 641)) == NULL) {
    free(list);
    return 0;
  }
