...
void ppu_draw_background_scanline(bool mirror) {
  int tile_x;
  for (tile_x = ppu_shows_background_in_leftmost_8px() ? 0 : 1; tile_x < 32;
       tile_x++) {
    ...
    int tile_index = ppu_ram_read(ppu_base_nametable_address() + tile_x +
                                  (tile_y << 5) + (mirror ? 0x400 : 0));
    word tile_address =
        ppu_background_pattern_table_address() + 16 * tile_index;
    ...
    for (x = 0; x < 8; x++) {
      ...
      if (color != 0) {
        ...
        word attribute_address =
            (ppu_base_nametable_address() + (mirror ? 0x400 : 0) + 0x3C0 +
             (tile_x >> 2) + (ppu.scanline >> 5) * 8);
        ...
        pixbuf_add(bg, (tile_x << 3) + x - ppu.PPUSCROLL_X + (mirror ? 256 : 0),
                   ppu.scanline + 1, idx);
      }
    }
  }
...
void ppu_draw_sprite_scanline() {
  ...
  for (n = 0; n < 0x100; n += 4) {
    ...
    if (sprite_y > ppu.scanline ||
        sprite_y + ppu_sprite_height() < ppu.scanline)
      continue;
    ...
    word tile_address =
        ppu_sprite_pattern_table_address() + 16 * PPU_SPRRAM[n + 1];
    int y_in_tile = ppu.scanline & 0x7;
    byte l = ppu_ram_read(tile_address + (vflip ? (7 - y_in_tile) : y_in_tile));
    byte h =
        ppu_ram_read(tile_address + (vflip ? (7 - y_in_tile) : y_in_tile) + 8);

    byte palette_attribute = PPU_SPRRAM[n + 2] & 0x3;
    word palette_address = 0x3F10 + (palette_attribute << 2);
    int x;
    for (x = 0; x < 8; x++) {
      int color = hflip ? ppu_l_h_addition_flip_table[l][h][x]
                        : ppu_l_h_addition_table[l][h][x];
  ...
  for (h = 0; h < 0x100; h++) {
    for (l = 0; l < 0x100; l++) {
      for (x = 0; x < 8; x++) {
        ppu_l_h_addition_table[l][h][x] =
            (((h >> (7 - x)) & 1) << 1) | ((l >> (7 - x)) & 1);
        ppu_l_h_addition_flip_table[l][h][x] =
            (((h >> x) & 1) << 1) | ((l >> x) & 1);
      }
    }
  }
...
void ppu_sprram_write(byte data) { PPU_SPRRAM[ppu.OAMADDR++] = data; }

void ppu_set_background_color(byte color) { nes_set_bg_color(color); }
