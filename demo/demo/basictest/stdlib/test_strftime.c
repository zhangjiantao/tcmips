...
int
#ifdef TESTALL
test_stdlib_strftime(int argc, char**argv)
#else
main(void)
#endif
{
  ...
  for (l = 0; l < sizeof(List) / sizeof(List[0]); l++) {
    ...
    for (i = 0; i < test->cnt; i++) {
      ...
      if (ret != test->vec[i].ret) {
        ...
        fprintf(
            stderr,
            "ERROR:  return %zd != %zd expected for List[%zd].vec[%d] \"%" SFLG
            "s\"\n",
  ...
  for (l = 0; l < sizeof(List) / sizeof(List[0]); l++) {
    ...
    for (i = 0; i < test->cnt; i++) {
      ...
      if (ret != 0) {
        ...
        fprintf(
            stderr,
            "ERROR:  return %zd != %d expected for List[%zd].vec[%d] \"%" SFLG
            "s\"\n",
  ...
  for (l = 0; l < sizeof(ListYr) / sizeof(ListYr[0]); l++) {
    ...
    for (i = 0; i < test->cnt; i++) {
      ...
      if (ret != test->vec[i].ret) {
        errr++;
        fprintf(stderr,
                "ERROR:  return %zd != %zd expected for ListYr[%zd].vec[%d] "
                "\"%" SFLG "s\"\n",
                ret, test->vec[i].ret, l, i, test->vec[i].fmt);
      } else {
        tcm_ascii_console_set_color(0, 0, 0, 0, 0xff, 0);
