//
// Created by zhangjiantao on 2026/5/11.
//

#include <dev/console.h>

#include <stdint.h>
#include <stdio.h>
#include <string.h>

// Test result macros
#define TEST_PASS(fmt, ...)                                                    \
  do {                                                                         \
    tcm_ascii_console_set_color(0, 0, 0, 0, 0xff, 0);                          \
    printf("[PASS] " fmt "\n", ##__VA_ARGS__);                                 \
    tcm_ascii_console_set_color(0xff, 0xff, 0xff, 0, 0, 0);                    \
  } while (0)
#define TEST_FAIL(fmt, ...)                                                    \
  do {                                                                         \
    tcm_ascii_console_set_color(0, 0, 0, 0xff, 0, 0);                          \
    printf("[FAIL] " fmt "\n", ##__VA_ARGS__);                                 \
    tcm_ascii_console_set_color(0xff, 0xff, 0xff, 0, 0, 0);                    \
  } while (0)

void sub_bytes(uint8_t *state) {
  static const uint8_t sbox[256] = {
      };
  for (int i = 0; i < 16; i++)
    state[i] = sbox[state[i]];
}

void shift_rows(uint8_t *state) {
  uint8_t tmp[16];
  tmp[0] = state[0];
  tmp[1] = state[5];
  tmp[2] = state[10];
  tmp[3] = state[15];
  tmp[4] = state[4];
  tmp[5] = state[9];
  tmp[6] = state[14];
  tmp[7] = state[3];
  tmp[8] = state[8];
  tmp[9] = state[13];
  tmp[10] = state[2];
  tmp[11] = state[7];
  tmp[12] = state[12];
  tmp[13] = state[1];
  tmp[14] = state[6];
  tmp[15] = state[11];
  memcpy(state, tmp, 16);
}

uint8_t gmul(uint8_t a, uint8_t b) {
  uint8_t p = 0;
  for (int i = 0; i < 8; i++) {
    if (b & 1)
      p ^= a;
    uint8_t hi = a & 0x80;
    a <<= 1;
    if (hi)
      a ^= 0x1b;
    b >>= 1;
  }
  return p;
}

void mix_columns(uint8_t *state) {
  uint8_t tmp[16];
  for (int i = 0; i < 4; i++) {
    int o = i * 4;
    tmp[o] =
        gmul(state[o], 2) ^ gmul(state[o + 1], 3) ^ state[o + 2] ^ state[o + 3];
    tmp[o + 1] =
        state[o] ^ gmul(state[o + 1], 2) ^ gmul(state[o + 2], 3) ^ state[o + 3];
    tmp[o + 2] =
        state[o] ^ state[o + 1] ^ gmul(state[o + 2], 2) ^ gmul(state[o + 3], 3);
    tmp[o + 3] =
        gmul(state[o], 3) ^ state[o + 1] ^ state[o + 2] ^ gmul(state[o + 3], 2);
  }
  memcpy(state, tmp, 16);
}

void add_round_key(uint8_t *state, const uint8_t *key) {
  for (int i = 0; i < 16; i++)
    state[i] ^= key[i];
}

void key_expansion(const uint8_t *key, uint8_t *exp_key) {
  static const uint8_t rcon[] = {0x01, 0x02, 0x04, 0x08, 0x10,
                                 0x20, 0x40, 0x80, 0x1b, 0x36};
  static const uint8_t sbox[256] = {
      };
  memcpy(exp_key, key, 16);
  for (int i = 16; i < 176; i += 4) {
    uint8_t t[4];
    memcpy(t, &exp_key[i - 4], 4);
    if (i % 16 == 0) {
      uint8_t x = t[0];
      t[0] = sbox[t[1]] ^ rcon[i / 16 - 1];
      t[1] = sbox[t[2]];
      t[2] = sbox[t[3]];
      t[3] = sbox[x];
    }
    for (int j = 0; j < 4; j++)
      exp_key[i + j] = exp_key[i - 16 + j] ^ t[j];
  }
}

void aes128_encrypt(uint8_t *state, const uint8_t *key) {
  uint8_t exp_key[176];
  key_expansion(key, exp_key);
  add_round_key(state, exp_key);
  for (int i = 1; i < 10; i++) {
    sub_bytes(state);
    shift_rows(state);
    mix_columns(state);
    add_round_key(state, &exp_key[i * 16]);
  }
  sub_bytes(state);
  shift_rows(state);
  add_round_key(state, &exp_key[160]);
}

void print_hex(const char *label, uint8_t *data, int len) {
  printf("%s: ", label);
  for (int i = 0; i < len; i++)
    printf("%02x", data[i]);
  printf("\n");
}

#ifdef TESTALL
int test_aes_main(int argc, char **argv) {
#else
int main(int argc, char **argv) {
#endif
  struct {
    uint8_t k[16];
    uint8_t p[16];
    uint8_t expected[16];
  } cases[] = {{{//
                 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
                {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
                {0x66, 0xe9, 0x4b, 0xd4, 0xef, 0x8a, 0x2c, 0x3b, 0x88, 0x4c,
                 0xfa, 0x59, 0xca, 0x34, 0x2b, 0x2e}},
               {{0x2b, 0x7e, 0x15, 0x16, 0x28, 0xae, 0xd2, 0xa6, 0xab, 0xf7,
                 0x15, 0x88, 0x09, 0xcf, 0x4f, 0x3c},
                {0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40, 0x9f, 0x96, 0xe9, 0x3d,
                 0x7e, 0x11, 0x73, 0x93, 0x17, 0x2a},
                {0x3a, 0xd7, 0x7b, 0xb4, 0x0d, 0x7a, 0x36, 0x60, 0xa8, 0x9e,
                 0xca, 0xf3, 0x24, 0x66, 0xef, 0x97}}};

  for (int i = 0; i < 2; i++) {
    uint8_t buffer[16];
    memcpy(buffer, cases[i].p, 16);
    aes128_encrypt(buffer, cases[i].k);

    printf("Test Case %d: \n", i + 1);

    print_hex("Key:      ", cases[i].k, 16);
    print_hex("Plain:    ", cases[i].p, 16);
    print_hex("Cipher:   ", buffer, 16);
    print_hex("Expected: ", cases[i].expected, 16);

    if (memcmp(buffer, cases[i].expected, 16) == 0) {
      TEST_PASS("PASS\n");
    } else {
      TEST_FAIL("FAIL\n");
    }
  }
  return 0;
}
